...
def build_and_evaluate_model(df: pd.DataFrame):
    ...
    try:
        rf_fitted = model.clf.estimator_
    except AttributeError:
        rf_fitted = model.named_steps.get("rf", None)

    importances = (
        pd.Series(rf_fitted.feature_importances_, index=FEATURE_COLS).sort_values(ascending=False)
        if rf_fitted and hasattr(rf_fitted, "feature_importances_")
        else pd.Series(np.zeros(len(FEATURE_COLS)), index=FEATURE_COLS)
    )
